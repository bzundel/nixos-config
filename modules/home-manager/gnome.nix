...
in
{
  options.gnome = {
    ...
    appearance = mkOption {
      ...
      lightMode = mkOption { # org/gnome/desktop/interface/color-scheme ("prefer-dark" or "prefer-light")
        type = types.bool;
        default = true;
        description = ''
      ...
      showBatteryPercentage = mkOption { # org/gnome/desktop/interface/show-battery-percentage
        type = types.bool;
        default = false;
        description = ''
      ...
      titlebarButtons = mkOption { # org/gnome/desktop/wm/preferences/button-layout (use optionalString to create setting dynamically)
        type = types.submodule;
    ...
    multitasking = mkOption {
      ...
      options = {
        hotCorner = mkOption { # org/gnome/desktop/interface/enable-hot-corners
          type = types.bool;
          default = true;
          description = "Enables hot corner to open activities overview.";
        };

        activeScreenEdges = mkOptions { # org/gnome/mutter/edge-tiling
          type = types.bool;
          default = false;
        ...
        workspaces = mkOption {
          ...
          options = {
            dynamicWorkspaces = mkOption { # org/gnome/mutter/dynamic-workspaces
              type = types.bool;
              default = true;
              description = "Use dynamic workspaces.";
            };

            numberOfWorkspaces = mkOption { # org/gnome/desktop/wm/preferences/num-workspaces
              ...
              apply = val: assert val >= 1 && val <= 36; val;
            };

            workspacesOnlyOnPrimary = mkOption { # org/gnome/mutter/workspaces-only-on-primary
              type = types.bool;
              default = false;
    ...
    power = mkOption {
      ...
      options = {
        dimScreen = mkOption { # org/gnome/settings-daemon/plugins/power/idle-dim
          
        };

        screenBlank = mkOption { # org/gnome/desktop/session/idle-delay
          type = types.ints.u32;
          default = 300;
        ...
        automaticSuspend = mkOption {
          ...
          options = {
            battery = mkOption {
              ...
              options = {
                enable = mkOption { # org/gnome/settings-daemon/plugins/power/sleep-inactive-battery-type ('suspend' when true, 'nothing' if false)
                  type = types.bool;
                  default = true;
                  description = "Enable automatic suspend when on battery power.";
                };

                delay = mkOption { # org/gnome/settings-daemon/plugins/power/sleep-inactive-battery-timeout
                  type = types.int;
                  default = 900;
            ...
            ac = mkOption {
              ...
              options = {
                enable = mkOption { # org/gnome/settings-daemon/plugins/power/sleep-inactive-ac-type ('suspend' when true, 'nothing' if false)
                  type = types.bool;
                  default = true;
                  description = "Enable automatic suspend when plugged in.";
                };

                delay = mkOption { # org/gnome/settings-daemon/plugins/power/sleep-inactive-ac-timeout
                  type = types.int;
                  default = 900;
